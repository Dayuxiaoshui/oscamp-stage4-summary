...
# 未完成的功能与后续计划
### `axfs_ramfs` (内存文件系统实现层): 性能与功能的打磨

不会影响 VFS 接口，专注于提升 `ramfs` 自身的内部实现，使其更快、更健壮、功能更完善。

* **核心修改文件**: `axfs_ramfs/src/lib.rs` (修改其内部数据结构、锁机制和方法实现)

* **改进方向**:

    * **性能与并发优化**:
        * **优化数据结构**: 将简单的 `Vec<u8>` 文件存储替换为**分块存储** (如 `Vec<[u8; BLOCK_SIZE]>`) 或 **B-Tree 映射**，以提高大文件的随机写性能并支持稀疏文件。
        * **细化锁粒度**: 将保护整个文件系统的全局锁，改为**每个文件/目录节点拥有自己的锁**，从而大幅提升并发操作的性能。

    * **功能与健壮性增强**:
        * **资源配额 (Quota)**: 增加内存使用量跟踪，防止 `ramfs` 无限制消耗系统资源。
        * **持久化支持**: 实现 `dump` 和 `load` 方法，允许将内存中的文件系统状态**序列化到磁盘**并恢复，使其在重启后数据不丢失。
        * **利用 `as_any` 实现特有功能**: 对于 `ramfs` 独有的功能（如碎片整理），无需修改 VFS 接口，直接在 `RamFsFile` 上实现，并通过 `node.as_any().downcast_ref()` 的方式进行调用。

### `axfs_devfs` (设备文件系统实现层): 设备生态的丰富

让设备文件系统支持更多标准虚拟设备，并从静态注册演进为动态管理。

* **核心修改文件**: `axfs_devfs/src/lib.rs` (管理设备注册与注销), `axfs_devfs/src/` 下创建新文件 (实现具体设备)。

* **改进方向**:

    * **扩充标准设备**: 创建新的设备模块，实现更多 `Unix-like` 系统的标准设备：
        * `/dev/random`, `/dev/urandom`: 返回随机数据。
        * `/dev/full`: 模拟磁盘满，写入时报错。
        * 未来可支持 TTY 设备等。

    * **实现动态设备管理**:
        * 改造 `DevFileSystem`，使其内部的设备列表线程安全。
        * 提供 `add_device` 和 `remove_device` 等公共方法，允许在**运行时动态地添加或移除设备**，而非在创建时就固定好。

### 也准备计划认领`kernel_guard` 

目前也已经阅读理解kernel_guard 提供了一系列遵循 RAII (Resource Acquisition Is Initialization) 设计模式的工具，用于在操作系统内核中创建临界区 (Critical Section)。进入临界区时，它可以自动地关闭中断或禁止内核抢占；离开临界区时（即代码块结束），它会自动恢复之前的状态。
